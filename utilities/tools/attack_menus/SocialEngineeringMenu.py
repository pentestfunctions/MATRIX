...
class SocialEngineeringMenu(Menu):
    def __init__(self, stdscr):
        super().__init__(stdscr)
        self.options = ["Phishing           | Not added", 
                        "Spear Phishing     | Not added", 
                        "Whaling            | Not added", 
                        "Beelogger          | Not added", 
                        "SEToolkit          | Not added", 
                        "Return to main menu"]
        self.title = "Social Engineering"
    ...
    def handle_input(self, c):
        ...
        if c in [curses.KEY_ENTER, 10, 13]:
            if self.current_selection == 0:
                from utilities.main_menu import MainMenu #Phishing
                return MainMenu(self.stdscr)
            
            elif self.current_selection == 1:
                from utilities.main_menu import MainMenu #Spear Phishing
                return MainMenu(self.stdscr)
            
            elif self.current_selection == 2:
                from utilities.main_menu import MainMenu #Whaling
                return MainMenu(self.stdscr)
            
            elif self.current_selection == 3:
                from utilities.main_menu import MainMenu #Beelogger
                return MainMenu(self.stdscr)
            
            elif self.current_selection == 4:
                from utilities.main_menu import MainMenu #SEToolkit
                return MainMenu(self.stdscr)
